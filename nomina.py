...
def sobre_tiempo():
    ...
    while not empleado:
        ...
        if empleado:
            entEmpleado =Obrero(empledo[1], empleado[2], empleado[3], empleado[4], empleado[5], empleado[6], empleado[7])
    ...
    if grabar == "s":
        ...
        if empAdm:
            ...
            for empleado in empAdm:
                entEmpAdm = Administrativo(empleado[1], empleado[2], empleado[3], empleado[4], empleado[5], empleado[6], empleado[7])
...
while opc != "4":
    borrar_Pantalla()
    menu =Menu("Menu Principal",["1) Mantenimeto","2) Novedades","3) Rol de Pago","4) Salir"])
    opc = menu.menu()
    if opc == "1":
        opc_1 = " "
        while opc_1 !="7"
        ...
        if opc_1 = "1":
            ...
            cargos()
            Menu("Mantenimiento de Cargos ",["1) Codigo:{}" "2) Descripcion Cargo:{}"])
        elif opc_1 == "4":
        ...
        while opc_1 != "3"
        borrar_Pantalla()
        menu_2 = Menu("Menu de Novedades",["1) Sobre Tiempo","2) Prestamo","3) Salir"])
        if opc_2 == "1":
            Sobre_Tiempo()
        elif opc_2 == "2":
            prestamo()
        ...
        borrar_Pantalla()
        menu_3 = Menu("Menu Rol",["1) Rol Administrativo","2) Rol Obrero","4) Salir"])
        if opc_3 = "1":
            rolAdministrativo()
        elif opc_3 == "2":
            rolObrero()
